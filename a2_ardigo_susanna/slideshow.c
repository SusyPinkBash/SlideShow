...
/* makes a copy of a given struct tag  */
...
/* gets the lenth of a string (end means ends with a 0 ASCII char) */
...
/* gets the photo struct give a photoID */
...
/* creates a linked list of two photos without any duplicate */
...
tags_for_scoring get_intersection_slide(struct slide * first, struct slide * second) {
    ...
    struct tag * dup_tag_this = NULL, * dup_tag_other = NULL;
    int n_this, n_other;
    ...
    if (first->orientation == V)
        tags_dup_delete(dup_tag_this);
    if (second->orientation == V)
        tags_dup_delete(dup_tag_other);
...
/* Constructor:
 * Creates a slide given a photoset and a photoID
 */
